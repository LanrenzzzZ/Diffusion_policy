...
class DiffusionModel(nn.Module):
    ...
    def conditional_sample(
        ...
        # times = torch.linspace(0., 1., self.steps, device = sample.device)
        t = torch.linspace(0, 1, self.steps + 1, device = sample.device)[:-1]
        dt = torch.ones(self.steps) / self.steps
        for step in range(self.steps):
            timesteps = torch.ones((batch_size,), device=sample.device) * t[step]
            ...
            sample = sample + model_output * dt[step]
        return sample
    ...
    def compute_loss(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        flow = trajectory - noise
        padded_times = rearrange(timesteps, 'b -> b 1 1')

        trajectory = noise.lerp(trajectory, padded_times)   # actions = (1- weight) * noise + weight * actions  weight=padded_times
        
        pred_trajectory_flow = self.unet(trajectory, timesteps, cond=global_cond)
        
        target = flow
        
        loss = F.mse_loss(pred_trajectory_flow, target, reduction="none")
...
class TransformerForActionDiffusion(ModuleAttrMixin):
    def __init__(self,
        ...
        # self.final_norm_softclamp = partial(softclamp, value = 30.)
